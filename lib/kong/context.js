...
module.exports = function connectionContext(username, password, adminUrl) {
  ...
  async function retrievalAdminRequest(resource, additionalConfiguration = {}, offset = null) {
    function standardKongResponseSemigroup(responseA, responseB) {
      const combinedData = responseA.data.concat(responseB.data);
      // preserve the later response's next cursor but with the combined information
    ...
    const result = await retrieveData(offset);
    if (result.next) {
      const nextResult = await retrievalAdminRequest(resource, additionalConfiguration, result.offset);
      return standardKongResponseSemigroup(result, nextResult);
    } else return result;
  }
